<?php 
add_action("wp_ajax_projects_ajax_callback", "projects_ajax_callback");
add_action("wp_ajax_nopriv_projects_ajax_callback", "projects_ajax_callback");

function projects_ajax_callback() {
	global $post;
	...
	if ($ids){
		foreach($ids as $single_project_id){
			...
			if ($number_of_projects == 6){
				$result['data'] .= '<div class="load-more-button-container white-icg-button">
